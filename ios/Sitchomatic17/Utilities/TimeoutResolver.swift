...
enum TimeoutResolver {
    private static let settings: AutomationSettings = AutomationSettings().normalizedTimeouts()

    static var shared: AutomationSettings { settings }

    static var userTestTimeout: TimeInterval {
        AutomationSettings.minimumTimeoutSeconds
    }

    static func resolveRequestTimeout(_ hardcoded: TimeInterval) -> TimeInterval {
        let pageLoad = settings.pageLoadTimeout
        if pageLoad > 0 {
